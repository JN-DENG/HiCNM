classdef plotLabels < handle
    ...
    methods
        ...
        function run(obj,fig_handle)
            plotData(obj.Data.Time, obj.Data.Data, obj.Data.T, obj.Data.crossing, obj.Data.T_switch);
        end
    end
end


function plotData(t, labels, Tlimits, crossing,t_special)
...
%     xticks_vals = [0:1000:M];
    xticks_vals = fix([0,t_special,Tlimits]);
end
if length(xticks_vals) > 2
    for iline=1:length(xticks_vals)-2
        plot([xticks_vals(iline+1),xticks_vals(iline+1)],[0,nCluster+1],'r','LineWidth',1)
    end
end
set(gca,'YTick',[0:5:nCluster+1]);
