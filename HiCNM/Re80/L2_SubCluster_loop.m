...
for i=1:20
    ...
    close all
    index_cluster_a=find( c_label==i);
    cut_point=find(diff(index_cluster_a)~=1)+1;  % cut points
    
    % Extract the snapshots in the cluster i
    cluster_a=a(index_cluster_a,:);
    ...
    for j=1:20
        ...
        plot(t0,a(:,j))
        hold on
        plot(t0(index_cluster_a),a(index_cluster_a,j),'.k')
        plot(dt*index_cluster_a(cut_point),x(cut_point,j),'or')
        axis tight
    end
%     saveas(1,['output/Input_fluc_C',num2str(i),'.png'])
    ...
    filename=['loop_Base20_C',num2str(i),'.mat'];
...
%     saveas(2,['output/centroids_C',num2str(i),'.png'])
    copyfile('output',['L2_C',num2str(i)])
end