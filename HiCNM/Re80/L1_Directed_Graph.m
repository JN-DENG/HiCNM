...
for i=1:length(row)
    ...
    if sum(sum([p1,p2]==[row,col],2)==2)==1
        ...
        x0 = xData(1 : 2);
        y0 = yData(1 : 2);
        x0(2)=x0(1)+0.75*(x0(2)-x0(1));
        y0(2)=y0(1)+0.75*(y0(2)-y0(1));
        xNew = posX + (x0 - minX) / (maxX - minX) * width;
        yNew = posY + (y0 - minY) / (maxY - minY) * height;
...
% text
for iCluster =1:nClusters  
   text(Vec_r(iCluster,2)+0.05, ...
        Vec_r(iCluster,1)-0.05,num2str(iCluster),'FontSize',18);
end
...
%%
name = sprintf('%s','./L1/2Dmapping.png');
print(1,name,'-dpng','-r300')
% name = sprintf('%s','./C20/2Dmapping.eps');
